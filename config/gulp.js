...
/**
 * Process Less files through Less and PostCSS.
 */
const styles = () => {
	const config = require('./postcss');
	const src = utils.srcStylesPath('style.less');
...
const copy = () => {
	const src = [
		...
		`!${utils.srcPath('partials/**')}`,
		`!${utils.srcPath('assets/{css,less,fonts,images,js,vendor}/')}`,
		`!${utils.srcPath('assets/{css,less,fonts,images,js,vendor}/**')}`,
		`!${utils.srcPath('assets/**/*.html')}`
	];
	const dest = utils.buildPath();
...
const watch = () => {
	gulp.watch(
		[utils.srcStylesPath('*.less'), utils.srcImagesPath('sprite/*.png')],
		styles
	);
